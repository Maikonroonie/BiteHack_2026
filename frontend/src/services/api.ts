 ...
 */
export async function analyzeFlood(request: AnalysisRequest): Promise<AnalysisResponse> {
 ...
 */
export async function getFloodMask(request: AnalysisRequest): Promise<AnalysisResponse> {
 ...
 */
export async function getBuildings(bbox: BoundingBox): Promise<BuildingsResponse> {
    const response = await api.post<BuildingsResponse>('/api/buildings', { bbox });
    return response.data;
}
