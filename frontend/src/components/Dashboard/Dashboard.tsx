  ...
  return (
    ...
    >
      <div className="flex items-start justify-between">
        ...
        <div
          className={`p-2 rounded-lg bg-orbital-surface ${colorClasses[color]}`}
        >
          {icon}
        </div>
      </div>
...
function estimateFinancialLoss(
  ...
  const infrastructureDamage =
    Math.ceil(floodedAreaKm2 / 2) * DAMAGE_COSTS.infrastructure;
...
║ Straty rolne:           ${(losses.agriculturalLoss / 1000000).toFixed(
    2
  )} mln PLN
║ ─────────────────────────────────────────────────────────────
...
Hackathon "BITEHACK" temat: "AI między orbitami" 2026
    `.trim();
}
    ...
    <div className="space-y-4">
      ...
      >
        <div className="flex items-center gap-2 mb-3">
          ...
          <h4 className="font-semibold text-cyber-yellow">
            Szacowane straty finansowe
          </h4>
        </div>
